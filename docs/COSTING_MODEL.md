...
|---|---|---|---|
| **1** | **Flag tiering + soft guardrail.** Split flags into *soft* (listed on Exceptions, still shipped on Price Upload) and *hard* (kept off the upload, as today). Move `OVER_MAX_INCREASE` / `OVER_MAX_DECREASE` to soft. Add a `review_note` / comment column to the review sheet so a human can object to a single row; a row with no note ships. | — | §8.C |
| **2** | **`markup_list` corrections.** `702-0026` / `602-0010` / `704-0014` Category → `Freeze`; `502-0007` Markup → `0.25`. Through a backed-up script (extend `scripts/fix_sale_units.py` or add `scripts/fix_markup_list.py`). | — | §4.3 |
| **3** | **My Cargo loader.** `my_cargo` block in `column_mapping.yaml`; loader reads first sheet, `My Cargo` filename prefix, one row per SKU; landed cost = D + F; `MYCARGO_UNIT_MISMATCH` (hard) when `unit` ≠ W10 base unit. No pricing change yet — `/check` learns to report it. | — | §5, §8.F/G |
